...
#' 
#' Conducts G-computation under the complete case missing value assumption
#' 
...
GcompCCMV <- function(chain, n_sim = NULL, j_0 = NULL, xi_f = function() 0, iterations = NULL) {
  if(is.null(iterations)) iterations <- 1:nrow(chain$beta)
  J <- chain$J
  if(is.null(j_0)) j_0 <- J
  
  f <- MakeFCCMV(chain, n_sim, j_0, J, xi_f)
...
MakeFCCMV <- function(chain, n_sim = NULL, j_0 = 8, J = 8, xi_f = function() 0) {
  ...
  f <- function(i) {
    g_comp <- with(chain, CCMVGcomp( N_sim = n_sim,
                                     J = J,
                                     j_0 = j_0,
                                     omega = omega[i,],
